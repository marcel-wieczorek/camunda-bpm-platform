...
public class ExecutionEntity extends VariableScopeImpl implements ActivityExecution, ExecutionListenerExecution, Execution, PvmExecution, ProcessInstance,
   ...
   * <li>an execution is split up into multiple concurrent executions, then the
   * parent is made inactive.</li>
  ...
  /**
   * next operation. process execution is in fact runtime interpretation of the
   * process model. each operation is a logical unit of interpretation of the
   * process. so sequentially processing the operations drives the
  ...
  public PvmProcessInstance createSubProcessInstance(PvmProcessDefinition processDefinition) {
    ...
    if (historyLevel >= ProcessEngineConfigurationImpl.HISTORYLEVEL_ACTIVITY) {
      ...
      // publish update event for sub process instance
      HistoryEvent haiue = eventFactory.getHistoricActivityInstanceUpdateEventProducer().createHistoryEvent(this);
      eventHandler.handleEvent(haiue);
  ...
  public void initialize() {
    ...
    // Cached entity-state initialized to null, all bits are zore, indicating NO
    // entities present
    cachedEntityState = 0;
  ...
  protected void scheduleAtomicOperationAsync(AtomicOperation executionOperation) {
    ...
    // At the moment, only AtomicOperationTransitionCreateScope can be performed
    // asynchronously,
  ...
  protected void ensureExecutionsInitialized() {
    if (executions == null) {
      this.executions = (List) Context.getCommandContext().getExecutionManager().findChildExecutionsByParentExecutionId(id);
    }
  }
  ...
  private void removeTasks(String reason) {
    ...
    for (TaskEntity task : getTasks()) {
      if (replacedBy != null) {
        ...
        Context.getCommandContext().getTaskManager().deleteTask(task, reason, false);
      }
    }
  }
  ...
  public void setReplacedBy(InterpretableExecution replacedBy) {
    ...
    for (TaskEntity task : getTasks()) {
      ...
      List<VariableInstanceEntity> variables = (List) commandContext.getVariableInstanceManager().findVariableInstancesByTaskId(task.getId());
  ...
  protected void updateActivityInstanceIdInHistoricVariableUpdate(HistoricDetailVariableInstanceUpdateEntity historicVariableUpdate,
    ...
    if (historyLevel >= ProcessEngineConfigurationImpl.HISTORYLEVEL_FULL && sourceActivityExecution != null) {
      // HistoricActivityInstanceEntity historicActivityInstance =
      // ActivityInstanceEndHandler.findActivityInstance(sourceActivityExecution);
  ...
  protected void ensureJobsInitialized() {
    if (jobs == null) {
      jobs = (List) Context.getCommandContext().getJobManager().findJobsByExecutionId(id);
    }
  }
  ...
  protected void ensureIncidentsInitialized() {
    if (incidents == null) {
      incidents = (List) Context.getCommandContext().getIncidentManager().findIncidentsByExecution(id);
    }
  }
